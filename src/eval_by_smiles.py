        ...
        for rxn in clean_rxns_postsani:
            ...
            if out:
                try:
                    ...
                    pred_mols[i] = Chem.MolFromSmiles(Chem.MolToSmiles(out[0][0], isomericSmiles=False))
                except Exception as e:
                    print(e)
                    print('Could not sanitize postsani reaction product: {}'.format(Chem.MolToSmiles(out[0][0], isomericSmiles=False)))
                    print('Original molecule was: {}'.format(Chem.MolToSmiles(mol, isomericSmiles=False)))
        ...
        for line in fpred:
            ...
            if bonds_as_doubles:
                ...
                for gedit in gedits.split(';'):
                    ...
                    cbonds.append((x - 1, y - 1, bond_types_as_double[t]))
            else:
                ...
                for gedit in gedits.split(';'):
                    x,y,t = gedit.split('-')
                    x,y = int(x), int(y)
                    if (x - 1,y - 1) in pbonds:
                        t = pbonds[(x - 1,y - 1)]
                    else:
                        t = 0
                    cbonds.append((x - 1, y - 1, t))
            ...
            if not psmiles <= pred_smiles:
                ...
                Chem.Kekulize(rmol)
                pred_smiles_kek = edit_mol_smiles(rmol, cbonds)
